...
Mmbs.Cloud.define("saveUser", async function(req, res) {
    try{
        ...
        ret = await user.save(obj, {useMasterKey: true})
        res.success(ret)
    } catch(err){
        res.error(err)
    }
...
Mmbs.Cloud.define("updateUser", async function(req, res) {
    try{
        ...
        let result = await user.save(obj, {useMasterKey: true})
        res.success(ret)
    } catch(err){
        res.error(err)
    }
...
Mmbs.Cloud.define("deleteUser", async function(req, res) {
    try{
        ...
        let obj = req.params;
        let user = Mmbs.User.createWithoutData(obj.id)
        let result = await user.destroy({useMasterKey: true})
        res.success(ret)
    } catch(err){
        res.error(err)
    }
 ...
 * 获取用户权限信息
 */
Mmbs.Cloud.define('getUserPermissions', async function(req, res) {
    try {
        let responseData = {permissions: []}
        let roles = await utils.getUserRoles(req.user)
        roles.map(role => {
            let permissions = role.attributes.permissions
            if (permissions) {
                permissions.forEach(p => { responseData.permissions.indexOf(p) === -1 && responseData.permissions.push(p) })
            }
        })
        // 查询菜单
        let SysMenus = Mmbs.Object.extend("SysMenus")
        let query = new Mmbs.Query(SysMenus)
        query.ascending("sort")
        query.containedIn('name', responseData.permissions)
        responseData.sysMenus = await query.find();
