...
const currentVersionFeatures = require('../package.json').mmbsDashboardFeatures;

var newFeaturesInLatestVersion = [];
// packageJson('mmbs-dashboard', 'latest').then(latestPackage => {
//   if (latestPackage.mmbsDashboardFeatures instanceof Array) {
//     newFeaturesInLatestVersion = latestPackage.mmbsDashboardFeatures.filter(feature => {
...
module.exports = function(config, options) {
  ...
  app.on('mount', function() {
    ...
    app.get('/dashboard-config.json', function(req, res) {
      ...
      if (!requestIsLocal && !req.secure && !options.allowInsecureHTTP) {
        ...
        return res.send({ success: false, error: 'Mmbs Dashboard can only be remotely accessed via HTTPS' });
      }

      if (!requestIsLocal && !users) {
        //Accessing the dashboard over the internet can only be done with username and password
        return res.send({ success: false, error: 'Configure a user to access Mmbs Dashboard remotely' });
      }
    ...
    // running dashboard from globally installed npm.
    ...
    app.get('/login', csrf(), function(req, res) {
      ...
      res.send(`<!DOCTYPE html>
        <head>
          ...
          <base href="${mountPath}"/>
          <script>
            MMBS_DASHBOARD_PATH = "${mountPath}";
          </script>
        </head>
        <html>
          <title>元图后端服务平台</title>
          <body>
